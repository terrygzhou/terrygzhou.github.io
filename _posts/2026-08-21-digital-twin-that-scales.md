...
First version was a snapshot — a frozen capture of my knowledge. I cycled through **AnythingLLM**, **OpenNotebook**, and **llm_wiki**. All powerful, all with the same flaw: they demanded structure before delivering value.

- **AnythingLLM** — solid document ingestion, multiple workspaces, but cross-project queries were painfully slow
- **OpenNotebook** — research-grade but over-engineered. Answering a quick question felt like filling out a form
- **llm_wiki** — the most ambitious. Connected concepts, not just documents. I refactored it into a web server so I could reach it from any device. But syncing Obsidian notes meant duplicating my knowledge graph. Two systems, two graphs, no merge path
...
I consolidated all my notes, photos, docs, emails, and experience into a single vector database: **Qdrant**. One collection, one query interface, one source of truth.
...
A good memory doesn't remember everything — it remembers what matters. I added **Mem0**, an adaptive memory layer that sits in front of my vector store and decides what to surface, and when.
...
Two paths: Microsoft's framework or a custom build. Microsoft's is powerful but heavy and compute-intensive. Custom requires more upfront work but delivers exactly what you need without unused baggage.

I went custom:

- **Existing stack** — the twin already had a body (Qdrant) and a brain (AI model). Adding Neo4j gave it a nervous system — the connections between everything — without a new framework layer
...
**Sourcing.** Ten channels: Telegram, VS Code sessions, Obsidian notes, manual injections, automated research cron jobs, Gmail and Yahoo emails, Apple Notes, personal PDFs, corporate files, web content. Each source has its own ingestion script.
...
The four-phase evolution — static snapshot, memory, learning, understanding — is one concrete answer. Each phase solved a real bottleneck from the previous one. Started as a photograph. Grew a memory. Learned to remember selectively. Learned to see connections between facts.

Adaptive memory that learns from usage, a knowledge graph that reveals hidden patterns, semantic search for fast recall — all on a local model. And I built the GraphRAG piece myself rather than adopting Microsoft's framework, because control and simplicity mattered more than ceremony. No cloud API token tax.

Your data stays yours. Your costs scale with compute, not tokens. And the quality gap keeps closing every time an open-source model ships. The twin gets smarter without ever phoning home.

Let's talk. What's the first thing you'd build into a local AI memory stack that a cloud API can't replicate?

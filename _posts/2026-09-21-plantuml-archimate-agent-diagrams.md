...
2. **Standard semantics** — two standards matter. The Open Group's ArchiMate spec defines element categories (Business, Application, Technology, Motivation, Strategy, Physical) and relationship types (Serving, Realization, Specialization, Composition, Association, Flow, Access, Aggregation, Influence, Triggering). OMG UML (class, sequence, activity, component, deployment) defines the structural and behavioural diagrams — class, sequence, activity, component, deployment — that agents need to document application interfaces, message flows, and runtime topology. The renderer must honor both, not just draw boxes.
...
- **Accurate structure and semantics for AI agents** — the ArchiMate stereotypes, relationship macros, and layer color coding encode Open Group semantics; the UML stereotypes and multiplicity syntax encode OMG UML semantics. An LLM can read, reason over, and use both as ground truth when generating downstream artifacts. The diagram's source is not just text; it is a standards-constrained representation that carries the meaning of a business process, an application service, a data store, and an interface sequence distinct from one another.
